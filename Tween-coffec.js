...
TWEEN.Interpolation = 
	Linear: (v,k) ->
		...
		if k < 0 then return fn v[ 0 ], v[ 1 ], f 
		if k > 1 then return fn v[ m ], v[ m - 1 ], m - f 
	...
	CatmullRom: ( v, k ) ->
		...
		if v[ 0 ] is v[ m ]
			if  k < 0  then i = Math.floor( f = m * ( 1 + k ) )
			return fn v[ ( i - 1 + m ) % m ], v[ i ], v[ ( i + 1 ) % m ], v[ ( i + 2 ) % m ], f - i
		else
			if k < 0 then return v[ 0 ] - ( fn( v[ 0 ], v[ 0 ], v[ 1 ], v[ 1 ], -f ) - v[ 0 ] )
			if k > 1 then return v[ m ] - ( fn( v[ m ], v[ m ], v[ m - 1 ], v[ m - 1 ], f - m ) - v[ m ] )
